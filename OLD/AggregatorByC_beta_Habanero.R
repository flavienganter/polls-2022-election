...
data <- data %>% 
  ...
  group_by(id_hyp) %>% mutate(id_hyp = cur_group_id()) %>% ungroup()
...
data_spline_model <- list(N             = nrow(data),
                          id_cand       = args[1],
                          tot_eff       = data$weighted_n,
                          id_hyp        = data$id_hyp,
                          H             = length(unique(data$id_hyp)),
                          censored      = data$censored,
                          bound_l       = data$bound_l,
                          bound_r       = data$bound_r,
...
# Run model
aggregator_model <- stan(file = "ModelSplinesByC_copy.stan",
