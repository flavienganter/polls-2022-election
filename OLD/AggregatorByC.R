...
data <- read_excel("PollsData.xlsx") %>% 
  ...
  # Recode truncated values and switch to share
    ...
    ## truncation value).
  mutate(cens_bound = case_when(share == "T_0.5" ~ .5,
                                share == "T_1.5" ~ 1.5,
                                share == "T_1" ~ 1),
         share = case_when(share == "T_0.5" ~ 0.0025,
                           share == "T_1.5" ~ 0.0075,
                           share == "T_1" ~ 0.005,
  ...
  group_by(id_hyp) %>% 
  mutate(share_sum = sum(share),
         share_std = share / share_sum) %>%
  ungroup() %>% 
  ...
  mutate(weighted_n = n_t1 * n_wgt) %>% 
  
  # Ccenter and standardize the variable that indicates what respondents
  # are included in the estimates (all of them, only those who are certain to
  # vote, or a mix of both) + rolling indicator
  mutate(unsure_1 = scale_factor(variable = unsure)[[1]],
  ...
  mutate(field_period_lgth = id_date_end - id_date_start + 1,
         daily_n = round(weighted_n / field_period_lgth)) %>% 
    
  # Expand rows over each poll's field period
  uncount(field_period_lgth, .id = "id_expand") %>% 
  group_by(id_hyp, id_expand) %>% 
  mutate(id_house = cur_group_id()) %>% 
  ungroup() %>% 
  mutate(id_date = id_date_start + id_expand - 1) %>% 
  
  # Upper and lower bounds
  mutate(censored = ifelse(!is.na(cens_bound), 1, 0),
         bound_l = (share - rounding / (2 * 100)) / share_sum,
         bound_l = ifelse(bound_l <= 0 | censored == 1, .001, bound_l),
         bound_r = (share + rounding / (2 * 100)) / share_sum,
         bound_r = ifelse(censored == 1, cens_bound / 100, bound_r))
...
get_draws <- function(candidate) {
  load(paste0("ModelOutput/model_aggregator_", candidate, ".RData"))
